...
public class UserControllerTest {
    ...
    public void test_positive_user_adding() throws Exception {
        ...
        assertEquals("Электронная почта не может быть пустой и должна содержать символ @", errorResponse.getError());
        ...
        responseAsString = mockMvc.perform(post("/users")
                ...
                .getResponse()
                .getContentAsString();
        errorResponse = objectMapper.readValue(responseAsString, ErrorResponse.class);
        assertEquals("Логин не может быть пустым и содержать пробелы", errorResponse.getError());
        ...
        responseAsString = mockMvc.perform(post("/users")
                ...
                .getContentAsString();
        errorResponse = objectMapper.readValue(responseAsString, ErrorResponse.class);
        assertEquals("Дата рождения не может быть в будущем", errorResponse.getError());
    ...
    public void test_positive_user_update() throws Exception {
        ...
        assertEquals("Нет такого пользователя", apiErrorResponse.getError());
